...
def fetch_gold():
    ...
    # Exchange rate for international gold
...
def fetch_stocks():
    ...
    # Attempt: PSX official endpoint
    data = _get_json("https://dps.psx.com.pk/symbols")
    if data and isinstance(data, list) and len(data) > 5:
        src = "dps.psx.com.pk"
        for item in data:
            sym = item.get("symbol", "")
            if sym not in _STOCK_META:
                continue
            meta  = _STOCK_META[sym]
            price = float(item.get("current") or _FALLBACK_PRICES.get(sym, 100))
            prev  = float(item.get("ldcp")    or price)
            chg   = round((price - prev) / prev * 100, 2) if prev else _FALLBACK_CHANGES.get(sym, 0)
            stocks.append({
                "sym": sym, **meta,
                "p": round(price, 2),
                "d": chg,
                ...
                "vol": item.get("volume", "—"),
                "high": round(float(item.get("high") or price * 1.02), 2),
                "low":  round(float(item.get("low")  or price * 0.98), 2),
            })
