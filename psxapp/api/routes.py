...
def init():
    ...
    return _ok({
        ...
        "summary": {
            "kse100": sd.get("kse100", 90400), "kse100_chg": sd.get("kse100_chg", 2.6),
...
def ai_insight():
    ...
    try:
        ...
        msg = client.messages.create(
            ...
            max_tokens=700,
            system=(
                f"You are a senior Pakistani financial analyst. Today's date is "
                f"{datetime.now(timezone.utc).strftime('%B %Y')}. "
                "Give concise, data-driven analysis for Pakistani investors, based on the "
                "current figures given to you in the topic/context — do not assume older "
                "market levels. Use ₨ for PKR."
            ),
...
def crypto():
    ...
    try:
        from psxapp.data.fetchers_enhanced import fetch_crypto
        return _ok(fetch_crypto())
    except ImportError:
        from psxapp.data.fetchers import fetch_exchange
        # Fallback crypto data
        pkr_usd = fetch_exchange().get("pkr_usd", 278.40)
        return _ok({
            "bitcoin": {"usd": 68500, "pkr": round(68500 * pkr_usd, 2), "change_24h": 2.3},
            "ethereum": {"usd": 3400, "pkr": round(3400 * pkr_usd, 2), "change_24h": 1.8},
            "source": "fallback"
        })


@api_bp.route("/world-indices")
def world_indices():
    """Major world market indices"""
    try:
        from psxapp.data.fetchers_enhanced import fetch_world_indices
        return _ok(fetch_world_indices())
    except ImportError:
        return _ok({
            "indices": [
                {"name": "S&P 500", "value": 5850.0, "change": 0.8},
                {"name": "NASDAQ", "value": 18500.0, "change": 1.2},
                {"name": "Dow Jones", "value": 39800.0, "change": 0.5},
            ],
            "source": "fallback"
        })
