...
# ── Future returns projection (Monte Carlo) ───────────────────────────────────
@portfolio_bp.route("/<int:pid>/projection")
@jwt_required()
def projection(pid):
    p = Portfolio.query.filter_by(id=pid, user_id=_uid()).first()
    if not p:
        return _err("Portfolio not found", 404)
    if not p.holdings:
        return _err("Add holdings to this portfolio before projecting returns", 400)

    from psxapp.portfolio.projection import project_portfolio
    holdings = [
        {"asset_id": h.asset_id, "asset_type": h.asset_type, "value": h.buy_price * h.units}
        for h in p.holdings
    ]
    result = project_portfolio(holdings)
    if "error" in result:
