/* ── PAGES — complete rewrite ────────────────────────────────────────────────
   ...
   Exported as window.IL (InvestorLens) to avoid name collision.
   ─────────────────────────────────────────────────────────────────────────── */
...
const SECTORS = [
  {n:'Technology',  d:2.8,  w:7.2,  m:28.4, q:65,  cap:'₨142B', cnt:8,  col:'#00ff9d'},
  {n:'Cement',      d:1.2,  w:3.1,  m:15.2, q:38,  cap:'₨580B', cnt:20, col:'#f0b90b'},
  {n:'Fertilizer',  d:0.8,  w:2.8,  m:12.5, q:31,  cap:'₨890B', cnt:6,  col:'#02c076'},
  {n:'Banking',     d:1.4,  w:2.9,  m:9.1,  q:24,  cap:'₨2.1T', cnt:25, col:'#00c27a'},
  {n:'Energy',      d:1.6,  w:3.5,  m:10.2, q:28,  cap:'₨1.8T', cnt:18, col:'#f6465d'},
  {n:'Power',       d:0.5,  w:1.2,  m:4.8,  q:15,  cap:'₨420B', cnt:12, col:'#b8890a'},
];
const FUNDS = [
...
async function overview() {
  ...
  el.innerHTML = `
  <div class="stats-row">
    ${statCard('KSE-100', (live.kse100||90400).toLocaleString(), live.kse100_chg??2.6,'#02c076')}
    ${statCard('Gold / Tola', `₨${(live.gold_24k_tola||327000).toLocaleString()}`, undefined,'#f0b90b')}
    ${statCard('Silver / Tola', `₨${(live.silver_tola||3620).toLocaleString()}`, undefined,'#06b6d4')}
    ${statCard('PKR / USD', `₨${(live.pkr_usd||278.40).toFixed(2)}`, undefined,'#848e9c')}
    ${statCard('SBP Rate','17.50%',undefined,'#f59e0b','Policy rate')}
    ${statCard('CPI Inflation','12.6%',undefined,'#f6465d','YoY, latest available')}
  </div>
  ...
  ${aiPanel(`Pakistan overall financial markets, ${new Date().toLocaleDateString('en-US',{month:'long',year:'numeric'})} — KSE-100 at ${(live.kse100||90400).toLocaleString()}, gold ₨${(live.gold_24k_tola||327000).toLocaleString()}/tola, SBP rate 17.5%, inflation 12.6%, IMF program outlook`,'Market Overview')}`;
...
async function gold() {
  ...
  const live = window.App?.live || {};
  const g24 = live.gold_24k_tola || 327000;
  const silverTola = live.silver_tola || 3620;
  const gsRatio = silverTola ? (g24/silverTola).toFixed(1) : '—';
  const intlOz = live.pkr_usd ? Math.round(g24/2.6667/live.pkr_usd) : null;

  const el = $('content'); if (!el) return;
  el.innerHTML = `
  <div class="stats-row">
    ${statCard('Gold 24K / Tola', `₨${g24.toLocaleString()}`, undefined,'#f0b90b')}
    ${statCard('Gold / 10g', `₨${Math.round(g24*0.857).toLocaleString()}`, undefined,'#f0b90b', intlOz?`Int'l ~$${intlOz.toLocaleString()}/oz`:'')}
    ${statCard('Silver / Tola', `₨${silverTola.toLocaleString()}`, undefined,'#06b6d4')}
    ${statCard('Gold : Silver', `${gsRatio} : 1`, undefined,'#848e9c','Historic avg ~65:1')}
  </div>
  ...
  ${aiPanel(`Gold and silver investment Pakistan, ${new Date().toLocaleDateString('en-US',{month:'long',year:'numeric'})} — ₨${g24.toLocaleString()}/tola${intlOz?`, international ~$${intlOz.toLocaleString()}/oz`:''}, PKR correlation, inflation hedge, entry strategy`,'Gold & Silver')}`;
...
async function stocks() {
  ...
  ${aiPanel(`PSX KSE-100 at ${(live.kse100||90400).toLocaleString()} Pakistan ${new Date().getFullYear()} — sector picks, valuation, dividend stocks, strategy`,'PSX Stocks')}`;
...
async function portfolio() {
  ...
  function renderSingle(pf, st) {
    ...
    ${h.length?`<div class="card mb10" id="projection-card">
      <div class="card-hd"><div><div class="card-title">📈 Future Returns Projection</div>
      <div class="card-sub">Monte Carlo simulation · statistical illustration, not financial advice</div></div></div>
      <div class="card-body" id="projection-body">
        <div class="empty"><div class="empty-msg">Calculating projection…</div></div>
      </div>
    </div>`:''}
  ...
  function confColor(c) { return c==='High' ? '#02c076' : c==='Medium' ? '#f59e0b' : '#f6465d'; }

  async function loadProjection(pid) {
    const body = $('projection-body');
    if (!body) return;
    let data;
    try { data = await API.projection(pid); }
    catch { data = { error: 'Network error' }; }
    if (!body || !document.body.contains(body)) return; // page navigated away
    if (data.error) {
      body.innerHTML = `<div class="empty"><div class="empty-msg">⚠ ${data.error}</div></div>`;
      return;
    }
    const horizons = ['1y','3y','5y'].map(k => data.horizons[k]);
    body.innerHTML = `
      <div class="gauto mb10">
        ${horizons.map(hz => `
        <div style="background:var(--bg2);border:1px solid var(--brd);border-radius:var(--r);padding:12px">
          <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px">
            <span style="font-weight:700;font-size:13px;color:var(--t1)">${hz.horizon_years}-Year Horizon</span>
            <span class="pill" style="background:${confColor(hz.confidence)}22;color:${confColor(hz.confidence)};border:1px solid ${confColor(hz.confidence)}44">${hz.confidence} confidence</span>
          </div>
          <div class="mono" style="font-size:20px;font-weight:700;color:${hz.median_return_pct>=0?'#02c076':'#f6465d'}">
            ${hz.median_return_pct>=0?'+':''}${hz.median_return_pct}%
          </div>
          <div style="font-size:10px;color:var(--t2);margin-bottom:8px">Median projected return → ${fmtPKR(hz.median_value)}</div>
          <div style="font-size:11px;color:var(--t2);line-height:1.7">
            <div>Range (p10–p90): <span class="mono">${hz.p10_return_pct}%</span> to <span class="mono">${hz.p90_return_pct}%</span></div>
            <div>Probability of gain: <span class="mono" style="color:#02c076">${hz.probability_of_gain_pct}%</span></div>
          </div>
        </div>`).join('')}
      </div>
      <div style="font-size:10px;color:var(--t2);line-height:1.6;border-top:1px solid var(--brd);padding-top:8px">
        <div><strong>Method:</strong> ${data.method}</div>
        <div style="margin-top:4px">${data.holdings_used.map(h=>`${h.asset_id}: ${h.annualized_return_assumed_pct}%/yr, ${h.annualized_volatility_pct}% vol (${h.basis})`).join(' · ')}</div>
        <div style="margin-top:6px;color:#f59e0b">⚠ ${data.disclaimer}</div>
      </div>`;
  }
...
// Export to window
// ─────────────────────────────────────────────────────────────────────────────
window.IL = { overview, gold, stocks, funds, fixedIncome, reits, opportunities, portfolio, simulator, profile };
