...
(async () => {
  ...
  // Page registry — uses window.IL which is set by pages.js
  const PAGE_TITLES = {
  ...
  async function navigateTo(page) {
    const fn = window.IL?.[page === 'fixed' ? 'fixedIncome' : page];
    if (!fn) {
      const el = $('content');
      if (el) el.innerHTML = `<div class="empty"><div class="empty-icon">⚠️</div>
        <div class="empty-msg">Page function not found</div>
        <div class="empty-hint">IL.${page} is undefined — check pages.js loaded correctly</div></div>`;
      return;
    }
    ...
    if (subEl)   subEl.textContent   = `Pakistan Investment Intelligence · ${new Date().toLocaleDateString('en-US',{month:'long',year:'numeric'})}`;
  ...
  // Merge live server data into the page-level fallback constants (STOCKS, live.*)
  // in place, so every page reading those globals picks up real prices instead
  // of the March-2026-vintage numbers baked in at build time.
  function mergeLiveStocks(liveStocks) {
    if (!Array.isArray(liveStocks) || typeof STOCKS === 'undefined') return;
    liveStocks.forEach(ls => {
      const row = STOCKS.find(s => s.sym === ls.sym);
      if (!row) return;
      Object.assign(row, { p: ls.p, d: ls.d, w: ls.w, m: ls.m, q: ls.q, y: ls.y,
        pe: ls.pe, dv: ls.dv, cap: ls.cap, vol: ls.vol, high: ls.high, low: ls.low });
    });
  }
  ...
  async function initMarketData() {
    try {
      const [d, gold] = await Promise.all([API.init(), API.gold()]);
      window.App.state = { stocks:d.stocks||[], sectors:d.sectors||[], exchange:d.exchange||{}, summary:d.summary||{} };
      window.App.live = {
        kse100: d.summary?.kse100, kse100_chg: d.summary?.kse100_chg,
        gold_24k_tola: gold?.gold_24k_tola, silver_tola: gold?.silver_tola,
        pkr_usd: d.exchange?.pkr_usd, gold_source: gold?.source, stocks_source: d.stocks_source,
      };
      mergeLiveStocks(d.stocks);
    ...
    setInterval(async()=>{
      try{
        const sd=await API.stocks();
        if(sd?.stocks){
          window.App.state.stocks=sd.stocks;
          window.App.live.kse100=sd.kse100; window.App.live.kse100_chg=sd.kse100_chg;
          mergeLiveStocks(sd.stocks);
          buildTicker(sd.stocks);
          updateKSE(sd.kse100,sd.kse100_chg);
        }
      }catch{}
    },60000);
  }
